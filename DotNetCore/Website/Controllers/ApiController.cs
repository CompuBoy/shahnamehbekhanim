...
namespace Shahnameh.Controllers
{
    public class ApiController : Controller
    {
        ...
        public ApiController(DataModel model)
        {
            this.model = model;
        }
        ...
        public ActionResult Posts(string category, string narrator, int? from, int? count)
        {
            return this.DoWithDatabase(model =>
            {
                ...
                return Json(posts.Select(p => new { id = p.ID, title = p.Title, category = p.Category, narrator = p.Narrator, story = p.Story, }).ToList());
            });
        }
        ...
        public ActionResult PodcastPosts(string category, int? from, int? count)
        {
            return this.DoWithDatabase(model =>
            {
                ...
                return Json(posts.ToList().Select(p => new { id = p.ID, title = p.Title, category = p.Category, description = p.Description, story = p.Story, url = p.PodcastUrl, date = ToPersianDate(p.PublishDate), }).ToList());
            });
        }
        ...
        public ActionResult PodcastPost(Guid id)
        {
            return this.DoWithDatabase(model =>
            {
                ...
                return Json(posts.ToList().Select(p => new { id = p.ID, title = p.Title, category = p.Category, description = p.Description, story = p.Story, url = p.PodcastUrl, date = ToPersianDate(p.PublishDate), }).FirstOrDefault());
            });
        }
        ...
        public ActionResult Verses(long postId)
        {
            return this.DoWithDatabase(model =>
            {
                return Json(model.Verses.Where(v => v.PostID == postId).OrderBy(v => v.Index).Select(v => new { index = v.Index, part1 = new { start = v.Part1StartTime, end = v.Part1EndTime }, part2 = new { start = v.Part2StartTime, end = v.Part2EndTime } }).ToList());
            });
        }
         ...
         public ActionResult Persons(long postId)
        {
            return this.DoWithDatabase(model =>
            {
                return Json(model.Persons.Where(pe => pe.PostID == postId).Select(pe => new { name = pe.Name, verse = pe.VerseIndex, inPart1 = pe.IsPresentInPart1, inPart2 = pe.IsPresentInPart2, }).ToList());
            });
        }
        ...
        public ActionResult Phrases(long? postId, string startingWith, int? from, int? count)
        {
            return this.DoWithDatabase(model =>
            {
                ...
                phrases = phrases.OrderBy(ph => ph.VerseIndex).ThenByDescending(ph => ph.IsPresentInPart1);
                ...
                return Json(phrases.Select(ph => new { value = ph.Value, description = ph.Description, postId = ph.PostID, verse = ph.VerseIndex, inPart1 = ph.IsPresentInPart1, inPart2 = ph.IsPresentInPart2, }).ToList());
            });
        }
        ...
        public ActionResult Categories()
        {
            return this.DoWithDatabase(model =>
            {
                return Json(model.Posts.GroupBy(p =>  p.Category, (category, posts) => new { Category = category, MinPostId = posts.Min(p => p.ID), }).OrderBy(i => i.MinPostId).Select(i => i.Category).ToList());
            });
        }
